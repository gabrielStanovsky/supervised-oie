...
# Joint figure -- only core
DIR="../evaluations/figures/binary_test"
echo "Creating JOINT (binary test) figure in ${DIR}..."
mkdir -p $DIR
rm -f $DIR/*
python benchmark.py --gold=./oie_corpus/test.oie.orig\
       --out=$DIR/ClausIE.dat --tabbed=./systems_output/test/binary_test/clausie_test.txt
python benchmark.py --gold=./oie_corpus/test.oie.orig\
       --out=$DIR/OpenIE-4.dat --tabbed=./systems_output/test/binary_test/openie4_test.txt
python benchmark.py --gold=./oie_corpus/test.oie.orig\
       --out=$DIR/PropS.dat --tabbed=./systems_output/test/binary_test/props_test.txt
python benchmark.py --gold=./oie_corpus/test.oie.orig\
       --out=$DIR/RnnOIE.dat --tabbed=../evaluations/extractions/binary_test/joint.txt
python pr_plot.py --in=${DIR} --out=${DIR} --outputtype=png
